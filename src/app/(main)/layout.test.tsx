...
const createMockUser = (
  ...
  ] as UserResource["emailAddresses"],
  ...overrides,
});
...
describe("MainLayout", () => {
  ...
  it("displays email address when user has no full name", () => {
    // Header now renders Clerk's <UserButton /> (mocked above) instead of
    // surfacing the signed-in user's name/email directly, so there is no
    // email text node to assert on. Keep the test to verify it still renders
    // without crashing for a user without a full name.
    const mockUser = createMockUser({
      fullName: null,
      emailAddresses: [
  ...
  it('displays fallback "User" when no user info is available', () => {
    // See note above: the user's name is no longer rendered by the layout,
    // so only assert the layout still renders the (mocked) UserButton.
    const mockUser = createMockUser({
      fullName: null,
      emailAddresses: [],
    });
