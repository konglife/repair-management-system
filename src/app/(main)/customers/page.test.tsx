...
describe("CustomersPage", () => {
  ...
  beforeEach(() => {
    ...
    (api.customers.getTotalCount.useQuery as jest.Mock).mockReturnValue(
      mockTotalCountQuery
    );
    (
      api.customers.getNewCustomersThisMonth.useQuery as jest.Mock
    ).mockReturnValue(mockNewCustomersQuery);
  ...
  describe("Page Structure", () => {
    ...
    it("displays Add New Customer button", () => {
      ...
      expect(
        screen.getByRole("button", { name: /add new customer/i })
      ).toBeInTheDocument();
    });
  });
  ...
  describe("Loading States", () => {
    it("displays loading state when customers are loading", () => {
      ...
      // During loading the count card shows a spinner instead of a number,
      // and the table renders its own loading spinner row.
  ...
  describe("Add Customer Dialog", () => {
    it("opens dialog when Add New Customer button is clicked", () => {
      ...
      expect(
        screen.getByRole("heading", { name: /add new customer/i })
      ).toBeInTheDocument();
      expect(
        screen.getByText(
    ...
    it("displays form fields with correct labels and requirements", () => {
      ...
      expect(
        screen.getByPlaceholderText("Enter phone number (optional)")
      ).toBeInTheDocument();
      expect(
        screen.getByPlaceholderText("Enter address (optional)")
      ).toBeInTheDocument();
    });
  ...
  describe("Edit Customer Functionality", () => {
    describe("Edit Button Display", () => {
      it("displays edit buttons for each customer row", () => {
        ...
        }); // Edit buttons have no text, just icon
        // Filter out the "Add New Customer" button
        const editButtonsOnly = editButtons.filter(
          (button) =>
    ...
    describe("Edit Dialog Management", () => {
      ...
      it("clears form state when dialog is closed", () => {
        ...
        const editButtonsAgain = screen.getAllByRole("button", {
          name: /edit customer/i,
        });
        const editButtonsOnlyAgain = editButtonsAgain.filter(
          (button) =>
            button.querySelector("svg") && !button.textContent?.includes("Add")
        );
  ...
  describe("Utils Invalidation Pattern", () => {
    ...
    describe("Create Customer Invalidation", () => {
      it("calls utils invalidation methods on successful customer creation", async () => {
        ...
        (api.customers.create.useMutation as jest.Mock).mockImplementation(
          (options) => {
        ...
        expect(
          mockUtils.customers.getNewCustomersThisMonth.invalidate
        ).toHaveBeenCalled();
      });
    ...
    describe("Update Customer Invalidation", () => {
      it("calls utils invalidation methods on successful customer update", async () => {
        const mockOnSuccess = jest.fn();
        (api.customers.update.useMutation as jest.Mock).mockImplementation(
          (options) => {
            mockOnSuccess.mockImplementation(options.onSuccess);
            return { mutate: jest.fn(), isPending: false };
          }
      ...
      it("update mutation does not invalidate analytics queries unnecessarily", async () => {
        ...
        expect(mockUtils.customers.getAll.invalidate).toHaveBeenCalled();
        expect(
          mockUtils.customers.getTotalCount.invalidate
    ...
    describe("Utils Integration", () => {
      ...
      it("verifies invalidation methods are async functions", async () => {
        ...
        mockUtils.customers.getTotalCount.invalidate.mockResolvedValue(
          undefined
        );
        mockUtils.customers.getNewCustomersThisMonth.invalidate.mockResolvedValue(
          undefined
        );
