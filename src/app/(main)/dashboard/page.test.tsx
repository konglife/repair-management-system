...
const mockGetSummaryQuery = api.dashboard.getSummary
  .useQuery as jest.MockedFunction<typeof api.dashboard.getSummary.useQuery>;

const mockGetTrendDataQuery = api.dashboard.getTrendData
  .useQuery as jest.MockedFunction<typeof api.dashboard.getTrendData.useQuery>;

const mockGetTopProductsQuery = api.dashboard.getTopProducts
  .useQuery as jest.MockedFunction<
  typeof api.dashboard.getTopProducts.useQuery
>;

const mockGetLowStockAlertsQuery = api.dashboard.getLowStockAlerts
  .useQuery as jest.MockedFunction<
  typeof api.dashboard.getLowStockAlerts.useQuery
>;

const mockGetRecentActivitiesQuery = api.dashboard.getRecentActivities
  .useQuery as jest.MockedFunction<
...
const mockSummaryData = {
  ...
  grossProfit: 10700, // salesProfit (3200) + repairProfit (7500)
};
...
describe("Dashboard", () => {
  beforeEach(() => {
    ...
    mockGetTopProductsQuery.mockReturnValue({
      data: [],
      isLoading: false,
      error: null,
    } as any);
    mockGetLowStockAlertsQuery.mockReturnValue({
      data: [],
      isLoading: false,
      error: null,
    } as any);
    mockGetRecentActivitiesQuery.mockReturnValue({
      data: [],
      isLoading: false,
      error: null,
    } as any);
  ...
  describe("Dashboard Page Component Tests", () => {
    ...
    it("displays loading indicators during API calls", () => {
      ...
      const skeletons = loadingElements.filter(
        (el) =>
          el.className.includes("animate-pulse") &&
          el.className.includes("bg-muted")
      );
      expect(skeletons).toHaveLength(7); // 7 cards with loading states
    });
  ...
  describe("Summary Cards Tests", () => {
    it("displays all five metric cards with correct data", () => {
      ...
      // Check formatted monetary values (฿ per formatCurrency)
      ...
      expect(screen.getByText("฿15,000.00")).toBeInTheDocument(); // totalStockValue
      expect(screen.getByText("฿10,700.00")).toBeInTheDocument(); // grossProfit
    });
    ...
    it("handles empty/null data gracefully", () => {
      ...
      expect(zeroValues).toHaveLength(7);
    });
    ...
    it("has responsive grid layout for different screen sizes", () => {
      ...
      // Walk up the ancestor chain (closest) to find the 7-column cards grid.
      const gridContainer = expensesCard.closest('[class*="lg:grid-cols-7"]');
