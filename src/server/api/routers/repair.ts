...
export const repairRouter = createTRPCRouter({
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const repair = await ctx.db.$transaction(
        async (tx: Prisma.TransactionClient) => {
          // Validate products + deduct stock (รวมใน module เดียว — เคยซ้ำกับ sale.create)
          const products = await validateAndDeductStock(tx, input.usedParts);
          ...
          const usedPartsData = input.usedParts.map((part) => {
            const product = products.get(part.productId)!;
  ...
  getAnalytics: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const totalRevenue = repairs.reduce(
        (sum: number, repair: Repair) => sum + repair.totalCost,
        0
      );
      const averageRepairCost =
        totalRepairs > 0 ? totalRevenue / totalRepairs : 0;
      const totalLaborRevenue = repairs.reduce(
        (sum: number, repair: Repair) => sum + repair.laborCost,
        0
      );
      const totalPartsCost = repairs.reduce(
        (sum: number, repair: Repair) => sum + repair.partsCost,
        0
      );
