...
## Tasks / Subtasks
- [ ] Create PurchaseRecord data model and Prisma schema implementation (AC: 3)
  - [ ] Verify PurchaseRecord model exists in Prisma schema with all required fields
  - [ ] Ensure foreign key relationship to Product model is correctly defined
  - [ ] Run database migration if schema changes are needed
- [ ] Create tRPC purchase router with purchase recording operations (AC: 1, 2, 3, 4, 5)
  - [ ] Create src/server/api/routers/purchase.ts with purchaseRouter
  - [ ] Implement create procedure with Zod validation for productId, quantity, costPerUnit
  - [ ] Implement getByProduct procedure for fetching purchase history by productId (AC: 6)
  - [ ] Include weighted average cost calculation logic in create procedure (AC: 5)
  - [ ] Include product quantity update logic in create procedure (AC: 4)
  - [ ] Add purchaseRouter to main appRouter in src/server/api/root.ts
- [ ] Enhance Stock Management page with Purchase Recording section (AC: 1)
  - [ ] Add Purchase Recording section to existing src/app/(main)/stock/page.tsx
  - [ ] Extend existing tabbed layout to include "Record Purchase" tab
  - [ ] Ensure seamless integration with existing Categories, Units, and Products tabs
- [ ] Implement Purchase Recording UI components (AC: 1, 2, 6)
  - [ ] Create purchase form with Product dropdown, quantity input, and cost per unit input (AC: 2)
  - [ ] Implement product selection dropdown populated from existing products
  - [ ] Add purchase history view component showing all purchases for selected product (AC: 6)
  - [ ] Include proper validation for quantity (positive integers) and cost (positive numbers)
  - [ ] Add success/error feedback for purchase recording operations
- [ ] Integrate tRPC client for purchase operations (AC: 1, 2, 3, 4, 5, 6)
  - [ ] Use tRPC React hooks for purchase creation and history retrieval
  - [ ] Implement optimistic updates for better UX on successful purchases
  - [ ] Add loading states and error handling for all purchase operations
  - [ ] Ensure real-time updates to product quantity and average cost after purchase
- [ ] Create comprehensive unit tests for Purchase functionality
  - [ ] Test PurchaseRecord tRPC router procedures (create, getByProduct)
  - [ ] Test weighted average cost calculation logic with multiple scenarios
  - [ ] Test product quantity update logic
  - [ ] Test purchase UI components rendering and interaction
  - [ ] Test purchase form validation and error scenarios
  - [ ] Test integration between purchase recording and product updates
