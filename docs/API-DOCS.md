...
| Procedure             | Type  | Input                                       | หมายเหตุ                                                                               |
| --------------------- | ----- | ------------------------------------------- | -------------------------------------------------------------------------------------- |
| `getSummary`          | query | `{ dateRange: "today"\|"7days"\|"1month" }` | รายได้/ต้นทุน/กำไร/มูลค่าสต็อก (C6: `period`→`dateRange`, parse ผ่าน `parseDateRange`) |
| `getTrendData`        | query | `{ period: "last30days" }`                  | รายได้ vs ค่าใช้จ่ายรายวัน 30 วัน (นอกขอบ C6 — คนละ concept)                           |
| `getTopProducts`      | query | `{ dateRange }`                             | ขายดี 5 อันดับ (C6: migrate `period`→`dateRange`)                                      |
...
| Procedure           | Type                   | Input                                 | หมายเหตุ                         |
| ------------------- | ---------------------- | ------------------------------------- | -------------------------------- |
| `getMonthlySummary` | query (protected — C5) | `{ startDate, endDate }` (YYYY-MM-DD) | สรุปเดือน พร้อมข้อมูลร้าน + logo |
